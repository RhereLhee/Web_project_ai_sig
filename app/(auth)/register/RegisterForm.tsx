          ...
          <form onSubmit={handleSubmitForm} className="space-y-4" noValidate>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">
                รหัสแนะนำ{" "}
                {refCode
                  ? <span className="text-emerald-400 text-xs">ใช้รหัสจากลิงก์แนะนำ</span>
                  : <span className="text-gray-500">(ถ้ามี)</span>
                }
              </label>
              <div className="relative">
                <input
                  ...
                  onChange={(e) => { if (!refCode) setField('referralCode', e.target.value) }}
                  readOnly={!!refCode}
                  className={`${inputCls(false)} ${refCode ? 'pr-10 cursor-not-allowed opacity-75' : ''}`}
