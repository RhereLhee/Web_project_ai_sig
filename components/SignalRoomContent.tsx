...
// Cells for pairs the free plan forbids are visually gated instead of hidden —
// keeps the 6-cell grid intact so the UI still feels complete while making
// clear which pairs are behind the upgrade.
const FREE_ALLOWED_DISPLAY = new Set(['EURUSD', 'USDJPY'])
...
export function SignalRoomContent({ user }: SignalRoomContentProps) {
  ...
  // Check if symbol is enabled — AND allowed on the current plan.
  // Free users see disabled cells for forbidden pairs (see render block below).
  const isSymbolEnabled = useCallback((symbol: string) => {
    const upstreamEnabled = symbolConfigs[symbol]?.enabled ?? true
    if (!allowedPairs) return upstreamEnabled
    return upstreamEnabled && allowedPairs.includes(symbol)
  }, [symbolConfigs, allowedPairs])
        ...
        {SYMBOLS.map((symbol, i) => {
          ...
          return (
            <div key={symbol} className="bg-[#0a0a0a] relative" style={{ aspectRatio: '16/9' }}>
              ...
              {/* Overlay: distinguish "locked by free plan" vs "maintenance" */}
              {!isEnabled && (() => {
                const isFreeLocked =
                  freePlanActive && !FREE_ALLOWED_DISPLAY.has(DISPLAY_NAMES[i])
                if (isFreeLocked) {
                  return (
                    <div className="absolute inset-0 z-20 bg-black/90 flex flex-col items-center justify-center">
                      <div className="w-12 h-12 md:w-16 md:h-16 mb-3 rounded-full bg-zinc-900 flex items-center justify-center border border-zinc-700">
                      ...
                      <p className="text-zinc-400 text-[10px] md:text-xs mb-2">
                        ปลดล็อกเมื่อสมัครแพ็กเกจ
                      </p>
                    </div>
                  )
                }
